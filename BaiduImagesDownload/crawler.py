    ...
    def get_images_url(word: str, num: int, timeout: int = __CONCURRENT_TIMEOUT) -> (bool, bool, list):
        ...
        async def __fetch(pn: int) -> None:
            ...
            async with ClientSession(timeout=ClientTimeout(total=timeout)) as session:
                try:
                    async with session.get(Crawler.__BASE_URL, params=par, headers=Crawler.__HEADERS) as res:
                        if res.status == 200:
                            text = await res.text()
                            logger.debug(text)
        ...
        with urlopen((Crawler.__BASE_URL + '?%s') % urlencode(params)) as r:
            if r.status != 200:
                net = False
            else:
            ...
            try:
                async with session.get(obj_url, headers=headers, allow_redirects=False) as res:
                    if res.status == 200 and 'content-type' in res.headers:
                        allow, ext = await Crawler.__check_type(res.headers['content-type'].partition(';')[0].strip(),
                                                                rule)
